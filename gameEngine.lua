...
GameEngine = {
    new = function(group)
        tempGroup = group
        local calculate
        local seq = {"ant1","ant2","ant3","antBoss"}
        local type1
        local type2
        local type3
        local NORMAL_ANT = 1
        ...
        function aaa(event)
            ...
            --CreateAntBoss.new(group,seq[4])
--            CreateAntRoam.new(group,seq[1],200)
